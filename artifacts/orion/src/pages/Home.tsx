...
function getPosition(lat: number, lon: number, altKm: number) {
  const R_EARTH = 6371;
...
function Satellites({ 
  ...
  useEffect(() => {
    if (!meshRef.current || !satellites) return;
    ...
    satellites.forEach((sat, i) => {
      ...
      const colorStr = categoryColorMap[sat.category] || "#ffffff";
  ...
  }, [satellites, categoryColorMap, dummy, colorObj]);

  return (
    <instancedMesh 
      ref={meshRef} 
      args={[undefined, undefined, satellites?.length || 0]}
      onClick={(e) => {
        e.stopPropagation();
        const instanceId = e.instanceId;
        if (instanceId !== undefined && satellites[instanceId]) {
          onSelect(satellites[instanceId].noradId);
        }
      }}
      onPointerOver={(e) => {
        e.stopPropagation();
...
export default function Home() {
  ...
  const { data: satellites } = useListSatellites(
    listParams,
  ...
  return (
    <Shell>
          ...
          >
            <Canvas camera={{ position: [0, 15, 30], fov: 45 }}>
              ...
              {satellites && categories && (
                <Satellites satellites={satellites} categories={categories} onSelect={setSelectedNoradId} />
              )}
            </Canvas>
          </WebGLErrorBoundary>
        ) : (
        ...
        <div className="bg-card/80 border border-primary/30 p-5 backdrop-blur-md pointer-events-auto">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {categories?.map(c => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 shadow-sm" style={{ backgroundColor: c.color, boxShadow: `0 0 8px ${c.color}` }} />
      ...
      {/* Detail Panel */}
      <div className={`absolute top-0 right-0 h-full w-96 bg-card/95 backdrop-blur-xl border-l border-primary/30 transition-transform duration-500 transform ${selectedNoradId ? 'translate-x-0' : 'translate-x-full'} pointer-events-auto flex flex-col z-20 shadow-[-20px_0_50px_rgba(0,0,0,0.5)]`}>
        {selectedSatLoading ? (
          ...
          <>
            <div className="p-6 border-b border-primary/20 flex items-start justify-between bg-primary/5">
              <div>
                ...
                <h2 className="text-2xl font-bold tracking-[0.1em] text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]">{selectedSat.name}</h2>
              ...
              <button onClick={() => setSelectedNoradId(null)} className="text-muted-foreground hover:text-primary p-2 border border-transparent hover:border-primary/30 transition-colors bg-black/20 shrink-0">
            ...
            <div className="p-6 flex-1 flex flex-col gap-8 font-mono overflow-y-auto">
              <div>
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-4">
                  <div className="bg-black/40 p-2 border border-white/5">
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Altitude</div>
                    <div className="text-lg text-white">{selectedSat.alt.toFixed(2)} <span className="text-xs text-primary/50">km</span></div>
                  </div>
                  <div className="bg-black/40 p-2 border border-white/5">
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Velocity</div>
                    <div className="text-lg text-white">{selectedSat.velocity.toFixed(3)} <span className="text-xs text-primary/50">km/s</span></div>
                  </div>
                  <div className="bg-black/40 p-2 border border-white/5">
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Inclination</div>
                    <div className="text-lg text-white">{selectedSat.inclination.toFixed(2)}°</div>
                  </div>
                  <div className="bg-black/40 p-2 border border-white/5">
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Period</div>
                    <div className="text-lg text-white">{selectedSat.period.toFixed(1)} <span className="text-xs text-primary/50">min</span></div>
                  </div>
                  <div className="bg-black/40 p-2 border border-white/5">
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Apogee</div>
                    <div className="text-lg text-white">{selectedSat.apogee.toFixed(1)} <span className="text-xs text-primary/50">km</span></div>
                  </div>
                  <div className="bg-black/40 p-2 border border-white/5">
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Perigee</div>
                    <div className="text-lg text-white">{selectedSat.perigee.toFixed(1)} <span className="text-xs text-primary/50">km</span></div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-4">
                  <div>
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest">Latitude</div>
                    <div className="text-sm text-white mt-0.5">{selectedSat.lat.toFixed(4)}°</div>
                  </div>
                  <div>
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest">Longitude</div>
                    <div className="text-sm text-white mt-0.5">{selectedSat.lon.toFixed(4)}°</div>
                  </div>
                  <div>
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest">RCS Class</div>
                    <div className="text-sm text-white mt-0.5">{selectedSat.rcs || "UNKNOWN"}</div>
                  </div>
                  <div>
                    <div className="text-[9px] text-muted-foreground uppercase tracking-widest">Launch Year</div>
                    <div className="text-sm text-white mt-0.5">{selectedSat.launchYear || "UNKNOWN"}</div>
                  </div>
                </div>
              </div>
              
              <div className="mt-auto pt-6 border-t border-primary/20">
                <div className="text-[10px] text-primary mb-3 tracking-[0.2em]">
                  TWO-LINE ELEMENT SET (RAW)
                </div>
